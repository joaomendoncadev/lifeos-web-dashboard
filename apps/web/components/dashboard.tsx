...
import { CardGridSkeleton, ListRowsSkeleton, Skeleton } from "./skeleton";
...
export function Dashboard() {
  ...
  return <main className="main-content">
    <header className="topbar"><div><p className="eyebrow">{date}</p><h1>Olá, João.</h1><p className="subtitle">Seu sistema pessoal para decidir, executar e revisar.</p></div><Link className="quick-add" href="/inbox"><Plus size={18}/> Capturar</Link></header>
    ...
    {!loading ? <OnboardingChecklist data={data}/> : null}
    <section className="stats-grid stats-grid-five">
      {loading ? <CardGridSkeleton count={5} className="stat-card"/> : <>
        <article className="stat-card primary"><span>Tarefas abertas</span><strong>{data.openTasks}</strong><small>{data.completedTasks} concluídas</small></article>
    ...
    <div className="content-grid">
      <section className="panel priority-panel"><div className="panel-header"><div><span>Prioridades</span><h3>O que merece sua atenção</h3></div><Link href="/today">Abrir Hoje</Link></div><div className="task-list">{loading ? <ListRowsSkeleton count={3}/> : data.priorities.length ? data.priorities.map(task => <article className="task-row" key={task.id}><button className="check-button"><Circle size={20}/></button><div className="task-copy"><strong>{task.title}</strong><span>{task.context}</span></div><div className="task-meta"><span><Clock3 size={14}/> {task.duration}</span><em>{task.priority}</em></div></article>) : <div className="empty-state compact"><Check size={28}/><strong>Nada pendente</strong><p>Seu dashboard está limpo.</p></div>}</div></section>
      <section className="panel habits-panel"><div className="panel-header"><div><span>Consistência</span><h3>Hábitos de hoje</h3></div><Link href="/habits">Abrir hábitos</Link></div>{loading ? <Skeleton height={13} width="70%" style={{ margin: "8px 0" }}/> : <><div className="progress-track"><span style={{width:`${habitPercent}%`}}/></div><p className="muted">Você concluiu {data.habitsDone} de {data.habitsTotal} hábitos.</p></>}</section>
      <section className="panel projects-panel"><div className="panel-header"><div><span>Em movimento</span><h3>Projetos ativos</h3></div><Link href="/projects">Abrir workspaces</Link></div>{loading ? <div className="empty-state compact"><Skeleton width={28} height={28} radius="50%"/><Skeleton width="60%" height={14} style={{ marginTop: 12 }}/></div> : <div className="empty-state compact"><Sparkles size={28}/><strong>{data.activeProjects} projetos cadastrados</strong><p>O progresso é atualizado conforme as tarefas são concluídas.</p></div>}</section>
